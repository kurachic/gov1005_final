...
asset<-asset %>%
  ...
  # group_by(var) %>%
  # mutate(nf_mean = as.numeric(nf_mean), s_mean = as.numeric(s_mean), nf_sd = as.numeric(nf_sd), s_sd = as.numeric(s_sd), mean_diff = s_mean - nf_mean, se = sqrt((s_sd^2 / 67) + (nf_sd^2 / 310)), t_score = mean_diff / se) %>%
